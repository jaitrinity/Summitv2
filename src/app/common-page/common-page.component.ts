...
export class CommonPageComponent implements OnInit {
  ...
  viewApprovedBy : any = "";
  topFirstCheckpointDesc : any = "";
  topFirstCheckpointValue : any = "";
  transactionDetList = [];
  ...
  viewDetails(event){
    ...
    this.topFirstCheckpointValue = event.data.topFirstCheckpointValue;
    let jsonData = {
      loginEmpId : this.loginEmpId,
  ...
  exportData(){
    if(this.transactionHdrList.length == 0){
      alert("Export data not exist");
      return;
    }

    let activityId = "";
    for(let i=0;i<this.transactionHdrList.length;i++){
      activityId += this.transactionHdrList[i].transactionId;
      if(i < this.transactionHdrList.length-1){
        activityId += ",";
      }
    }
    ...
    window.open(Constant.phpServiceURL+'exportHdrAndDetData.php?jsonData='+JSON.stringify(jsonData));
